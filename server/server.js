...
//Creating middleware to fetch Seller
...
app.post('/addproduct',fetchSeller, async (req, res) => {
    ...
    let sellerData = await Sellers.findOne({ _id: req.seller.id });
    sellerData.prolistData[req.body.itemId] += 1;
    await Sellers.findOneAndUpdate({ _id: req.seller.id }, { prolistData: sellerData.prolistData });
    res.json({
        success: true,
        name: req.body.name,
    })
...
//Creating endpoint for registrating Seller
...
//Creating endpoint for Seller login
...
//Creating endpoint for popular collection data

app.get('/popularinhandicraft', async (req, res) => {
    let products = await Product.find({ category: "handicraft" });
    let popular_in_handicraft = products.slice(0, 4);
    console.log("Popular in Handicratft Fetched");
    res.send(popular_in_handicraft);
})


//Creating endpoint for products in cart
